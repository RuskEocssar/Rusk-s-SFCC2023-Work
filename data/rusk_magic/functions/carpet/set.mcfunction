...
## カーペット用のディスプレイを召喚
    # ベース
    summon silverfish ~ ~ ~ {Tags:[rusk_magic,magic_carpet,carpet_base,temp_base],CustomName:'{"text":""}',Team:"nocol",Silent:1b,NoGravity:1b,DeathLootTable:"",DeathTime:19s,Invulnerable:1b,ActiveEffects:[{Id:14,Duration:-1,ShowParticles:false}]}
    # カーペット部分
...
## エンティティ達の初期設定
    ...
    # スコア初期設定
    execute as @e[tag=temp_base] run function rusk_magic:carpet/flutter/init

## 後処理
    # タグ
    tag @e[tag=temp,limit=1] remove temp
    tag @e[tag=temp_base,limit=1] remove temp_base