...
## Motionを取得する
    ...
    data modify entity @e[tag=dragon_base,limit=1] Motion set from entity 0-0-1-0-2f19325aa Pos
    # 保存
    data modify storage _: saved_data.dragon_motion prepend from entity 0-0-1-0-2f19325aa Pos
    # 後処理
    kill 0-0-1-0-2f19325aa

## 向きを取得する
    # 取得
    data modify entity @e[tag=dragon_base,limit=1] Rotation[0] set from entity Rusk_Eocssar Rotation[0]
    # 保存
    data modify storage _: saved_data.dragon_rotation prepend from entity Rusk_Eocssar Rotation[0]

## 位置測定用
    execute as Rusk_Eocssar at @s positioned ^ ^ ^30 run particle explosion ~ ~ ~ 0 0 0 0 1 force
...
## ループ
    ...
    # ループ
    schedule function rusk_magic:record/dragon 1t
    # 
    execute if score _ time1 matches 1200.. run schedule clear rusk_magic:record/dragon
